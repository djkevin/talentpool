			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="skill.person.label" default="Person" /></th>
					
						<g:sortableColumn property="skillName" title="${message(code: 'skill.skillName.label', default: 'Skill Name')}" />
				...
				<g:each in="${skillInstanceList}" status="i" var="skillInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${skillInstance.id}">${fieldValue(bean: skillInstance, field: "person")}</g:link></td>
					
						<td>${fieldValue(bean: skillInstance, field: "skillName")}</td>
